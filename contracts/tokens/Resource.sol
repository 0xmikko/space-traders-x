...
contract ResourceToken is Context, IBEP20, Ownable {
    ...
    mapping(address => PlanetFossil) private _planetGenerates;

    uint256 private _currentGeneratePerBlock;
    uint256 private _generatedAtLastUpdate;

    uint256 private _blockLastUpdate;
    ...
    function addPlanet(
        ...
        emit NewPlanetAdded(planet, initValue, generatePerBlock);
    }

    // Calculates how much tokes were generated by plantes to current block
    function calculatesTotalGenerateValue() internal view returns (uint256) {
        return block.number.sub(_blockLastUpdate).mul(_currentGeneratePerBlock);
    }
    ...
    function totalSupply() external override view returns (uint256) {
        return
            _generatedAtLastUpdate
                .add(calculatesTotalGenerateValue())
                .add(_totalMinted)
                .sub(_totalBurned);
    }
    ...
    function getCurrentGeneratePerBlock() external view returns (uint256) {
        return _currentGeneratePerBlock;
    }

    function getGeneratedLastUpdate() external view returns (uint256) {
        return _generatedAtLastUpdate;
    }
